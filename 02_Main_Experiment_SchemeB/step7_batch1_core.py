"""Run core model comparison, spatial-validation, and residual autocorrelation analyses."""
...
def calc_moran_knn(coords, values, k=8):
    """Compute K-nearest-neighbor Moran's I without building a dense spatial-weights matrix."""
...
def calc_moran_distance(coords, values, threshold_km):
    """Compute distance-threshold Moran's I using approximate kilometer coordinates."""
    from scipy.spatial import cKDTree

    coords_km = coords.copy()
    coords_km[:, 0] *= 111.0
    coords_km[:, 1] *= 111.0 * np.cos(np.radians(np.mean(coords[:, 0])))
    n = len(values)
    z = values - np.mean(values)
