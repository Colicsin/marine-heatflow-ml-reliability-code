...
OUT_PATH = OUT_DIR / "step4_observed_points_robinson.png"
...
def build_cmap():
    ...
    return LinearSegmentedColormap.from_list("obs_points_paper", colors, N=256)


def main():
    df = pd.read_csv(DATA_PATH, usecols=["lat_NS", "long_EW", "q"])
    polygons = read_polygon_parts(LAND_SHP)

    x, y = robinson_project(df["long_EW"].values, df["lat_NS"].values)
    ...
    ax.set_title(
        "Observed Oceanic Heat Flow (28,642 discrete observations)",
        fontsize=17,
        fontweight="bold",
        pad=12,
    )
